...
export class FaithEffectsService {
  ...
  async get(id: number) { const [row] = await this.ctx.database.get("faith_core_effects", { id }); return row ?? null; }
  remove(id: number) { return this.ctx.database.remove("faith_core_effects", { id }); }
